...
# status-all.sh [-l] # current location implied
# status-all.sh [-l] ~/my-projects /var/group-projects
...
find "$followLinks" "$@" -type d -name ".git" | while read -r file; do process_git_list "$file"; done
